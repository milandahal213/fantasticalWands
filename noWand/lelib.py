...
class singleMotor(_CardReader, le.SingleMotor):
    ...
    def connect(self, card_serial=None, card_color=None, device=None):
        for attempt in range(5):
            try:
                super().connect(device=device, card_color=card_color, card_serial=card_serial)
                break
            except Exception as e:
                if device or "not ready" not in str(e).lower() or attempt >= 4:
                    raise
                time.sleep(1)
