...
# ─────────────────────────────────────────────
# CARD COLOR / NUMBER DECODING
# ─────────────────────────────────────────────
# Page 5 on these cards is formatted as: 00 B1 B2 B3
#   B1       = color/category code
#   B2 B3    = card number (big-endian hex value)
#
# Fill in more entries as you discover them (e.g. yellow, green).
COLOR_CODES = {
    0x01: "Pink",
    0x02: "Purple",
    0x03: "Blue",
...
def decode_card(page5_bytes):
    """
    page5_bytes: 4 bytes read from NTAG page 5, e.g. b'\\x00\\x01\\x08\\xEB'
    Returns dict: {'color': str, 'number': int, 'code': int, 'raw': hex str}
    """
    if len(page5_bytes) != 4:
        raise ValueError("Expected exactly 4 bytes from page 5")

    b0, b1, b2, b3 = page5_bytes
    color_code = b1
    number = (b2 << 8) | b3
    color = COLOR_CODES.get(color_code, f"Unknown (code 0x{color_code:02X})")

    return {
        "color": color,
        "number": number,
        "code": color_code,
        "raw": " ".join(f"{b:02X}" for b in page5_bytes),
    }


def print_card(page5_bytes):
    info = decode_card(page5_bytes)
    print(f"  Card color/number -> {info['color']}, #{info['number']}  (raw: {info['raw']})")
    return info
...
def read_ntag(nfc, tag):
    ...
    # Decode color/number from page 5, if we got it
    if page5_bytes is not None:
        print()
        print_card(page5_bytes)
