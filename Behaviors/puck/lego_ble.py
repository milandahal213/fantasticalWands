...
class LegoDevice:
    ...
    def __init__(self, kind, label, send):
        ...
        self.pitch = None
        self.roll = None
        self.accel_x = self.accel_y = self.accel_z = None
        self.imu_gesture = None
        self.left = None
        self.right = None
        self.color = None
    ...
    def apply(self, subs):
        for s in subs:
            if s.type == _SUB_MOTOR:
                ...
                self.yaw = s.f["yaw"]
                self.pitch = s.f["pitch"]
                self.roll = s.f["roll"]
                self.accel_x, self.accel_y, self.accel_z = s.f["ax"], s.f["ay"], s.f["az"]
            elif s.type == 16:      # IMU gesture (tap/shake/collision/freefall)
                self.imu_gesture = s.f["gesture"]
    ...
    def beep(self, frequency=440, count=1):
        self._send(cmd_play_beep(pattern=0, frequency=frequency, count=count))
